...
#def chain(n, primary_in=0, primary_out=0):
#    s = gp.VectorVariable(n, "s")
#    l = gp.VectorVariable(n-1, "l")
#
#    d = 0
#    for i in xrange(n-1):
#        d += delay(i, n-1, s, l)
#    
#    s_min = gp.VectorVariable(n, "smin", [1 for dummy in xrange(n)])
#    s_max = gp.VectorVariable(n, "smax", [10 for dummy in xrange(n)])
#    l_min = gp.VectorVariable(n-1, "lmin", [0.001 for dummy in xrange(n-1)])
#    l_max = gp.VectorVariable(n-1, "lmax", [1 for dummy in xrange(n-1)])
#    constraints = []
#    l_sum = 0
#    for i in range(n):
#        constraints.append(s[i] <= s_max[i])
...
#    
#    if (primary_in > 0):
#        constraints.append(s[0] == primary_in)
#
#    if (primary_out > 0):
#        constraints.append(s[-1] == primary_out)
#
#    with gp.SignomialsEnabled():
#        constraints.append((l_sum >= 1))
#  
#    return gp.Model(d, constraints)
#
#def delay(i, n, s, l):
#    d = 0
#    d += (s[i+1] + l[i]) / s[i]
